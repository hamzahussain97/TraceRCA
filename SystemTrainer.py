# -*- coding: utf-8 -*-
"""
Created on Thu Apr 25 12:27:50 2024
...
import sys
from ModelTrainer import MAPE, percentile_mape, percentile_mae, plot, quantile_loss, multi_quantile_loss, calculate_metrics, plot_percentiles, coverage_probability
...
class SystemModelTrainer():
    ...
    def initialize_models(self, input_dim, gnn_hidden, gru_hidden, trace_gnn_hidden):
        ...
        self.trace_gnn = TraceGNN(gnn_hidden, trace_gnn_hidden, len(self.quantiles))
    ...
    def step(self, batch, loss_fn, criterion, optimizer):
        ...
        trace_graphs = batch.trace_graphs[0]
...
if __name__ == "__main__":
    ...
    quantiles = quantiles = [0.0013, 0.0228, 0.1587, 0.5000, 0.8413, 0.9772, 0.9987]
    model_trainer = SystemModelTrainer(path, 1, quantiles, features)
    model_trainer.initialize_models(12, 64, 64, 32)
    loss = torch.nn.MSELoss(reduction='mean')
    criterion = torch.nn.L1Loss(reduction='mean')
    model_parameters = list(model_trainer.gnn.parameters()) + list(model_trainer.trace_gnn.parameters())
